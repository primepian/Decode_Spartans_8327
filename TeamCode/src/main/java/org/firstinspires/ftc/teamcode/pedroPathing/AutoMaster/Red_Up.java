...
public class Red_Up extends OpMode{
    ...
    private final Pose shoot_Pose = new Pose(54, 90.000, Math.toRadians(137.5)).mirror();           //TL:Path #2 TODO: Shoot fixme:53.000, 90.000, Math.toRadians(135)
    ...
    public void autonomousPathUpdate() {
        ...
        switch (pathState) {
            case 0: //start to obelisk
                ...
                mecanism.B = 1;
                mecanism.C = 2;
                mecanism.shootPow(.67);//note: power ++++++
                break;
            ...
            case 27:
                if (follower.getPose().getY() > 8){
                    mecanism.shootPow(0.68); //note: Power +++++
                }
                if (!follower.isBusy()) {
