...
@Autonomous()
public class AutoRedA1 extends OpMode {
...
//    private final Pose obeliskPose = new Pose(60, 132, Math.toRadians(55)).mirror();         //PT: ROBOT IS SEEING THE OBELISK.
    private final Pose shootPose = new Pose(60, 132, Math.toRadians(180)).mirror();      //PT: Robot is horizontally seeing the blue goal.
    private final Pose pickIIIPose = new Pose(60, 84, Math.toRadians(180)).mirror();     //PT: Top (First Set) Blue.
    private final Pose pickedIIIPose = new Pose(18, 84, Math.toRadians(180)).mirror();   //PT: Top (First Set) Blue already picked.
    private final Pose release = new Pose(16, 70, Math.toRadians(90)).mirror();          //PT: Release Gate
    private final Pose releaseCP = new Pose(30, 70).mirror();
    private final Pose pickIIPose = new Pose(60, 60, Math.toRadians(180)).mirror();      //PT: Middle (Second Set) Blue.
    private final Pose pickedIIPose = new Pose(18, 60, Math.toRadians(180)).mirror();    //PT: Middle (Second Set) Blue already picked.
    private final Pose pickIPose = new Pose(60, 35, Math.toRadians(180)).mirror();       //PT: Low (First Set) Blue.
