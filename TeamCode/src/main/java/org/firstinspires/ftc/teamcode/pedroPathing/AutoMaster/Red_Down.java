...
public class Red_Down extends OpMode{
    ...
    private final Pose startingPose = new Pose(55, 9, Math.toRadians(90)).mirror();            //TL:Path #1
    private final Pose search_pose = new Pose(63, 30, Math.toRadians(87)).mirror();           //TL:Path #1

    private final Pose shoot_Pose = new Pose(60, 20, Math.toRadians(115)).mirror();           //TL:Path #2 TODO: Shoot fixme:53.000, 90.000, Math.toRadians(135)

    private final Pose fst_itk_pose = new Pose(45.000, 33, Math.toRadians(180)).mirror();          //TL:Path #3

    private final Pose fst_itk_1 = new Pose(33.000, 33.5, Math.toRadians(180)).mirror();             //TL:Path #4

    private final Pose fst_itk_2 = new Pose(28.000, 33.5, Math.toRadians(180)).mirror();               //TL:Path #5

    private final Pose fst_itk_3 = new Pose(18.000, 33.5, Math.toRadians(180)).mirror();             //TL:Path #6

    private final Pose snd_itk_pose = new Pose(45.000, 55.000, Math.toRadians(180)).mirror();          //TL:Path #8

    private final Pose snd_itk_1 = new Pose(33.000, 56.000, Math.toRadians(180)).mirror();             //TL:Path #9

    private final Pose snd_itk_2 = new Pose(28.000, 56.000, Math.toRadians(180)).mirror();             //TL:Path #10

    private final Pose snd_itk_3 = new Pose(18.000, 56.000, Math.toRadians(180)).mirror();             //TL:Path #11

    private final Pose end1 = new Pose(25.000, 66.000, Math.toRadians(180)).mirror();                               //TL:Path #12
        ...
        switch (pathState) {
            ...
            case 10:
                if (!follower.isBusy() && !mecanism.isShooting) {
                    mecanism.shootPow(0);
                    follower.followPath(trd_path, true); //fixme: follower.followPath(final_a, true);
                    setPathState(30);
                }
                break;
            case 30:
